...
export class StoryService {
    ...
    createStory(mediaUrl: string, caption?: string, songTitle?: string, songArtist?: string, songUrl?: string): Observable<ApiResponse<{ storyId: number }>> {
        let params = new HttpParams().set('mediaUrl', mediaUrl);
        if (caption) params = params.set('caption', caption);
        if (songTitle) params = params.set('songTitle', songTitle);
        if (songArtist) params = params.set('songArtist', songArtist);
        if (songUrl) params = params.set('songUrl', songUrl);
    ...
    getStoryViewers(storyId: number): Observable<ApiResponse<StoryViewer[]>> {
        return this.http.get<ApiResponse<StoryViewer[]>>(`${this.api}/${storyId}/viewers`);
    }
}
